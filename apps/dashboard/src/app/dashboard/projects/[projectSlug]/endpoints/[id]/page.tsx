...
import { getEndpointById, updateEndpoint } from '../../../../../actions';
...
export default function EditEndpointPage() {
  ...
  useEffect(() => {
    async function loadEndpoint() {
      try {
        const ep = await getEndpointById(endpointId);
        setPath(ep.path);
        setMethod(ep.method);
  ...
  if (loading) {
    return <div className="text-zinc-500 text-center py-20 font-medium">Loading endpoint details...</div>;
  }
      ...
      <div className="flex justify-between items-center">
        <div>
          <div className="flex items-center space-x-2 text-xs text-zinc-500 mb-1.5 font-medium">
            ...
            <span>/</span>
            <span className="text-zinc-300 font-semibold">Edit Mock Endpoint</span>
          </div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Modify Mock API</h1>
        </div>
      </div>
